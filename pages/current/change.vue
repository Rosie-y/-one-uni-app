...
<script>
	...
	export default {
		...
		methods: {
			getData() {
				uni.request({
					url: this.$serverUrl + '/article/today?dev=1',
					success: (res) => {
						var prev = res.data.data.date.prev;
						this.currentday = res.data.data.date.curr;
						console.log("currentday:" + this.currentday);
						uni.request({
							url: this.$serverUrl + '/article/day?dev=1&date= ' + prev,
							success: (res1) => {
								let obj = {
								...
								this.author=res1.data.data.author;
			...
			/* 跳转到next */
			next(e) {
				if (this.nex == this.currentday) {
					uni.navigateTo({
						url: './index'
					})
				} else {
					uni.request({
						url: 'https://interface.meiriyiwen.com/article/day?dev=1&date= ' + e,
						success: (res1) => {
							let obj = {
								curr: res1.data.data.date.curr,
								prev: res1.data.data.date.prev,
			...
			onClick(title,author) {
				...
				uni.getStorage({ //获取本地收藏数据,第一次获取因为还没有保存title键，会回调fail函数，可以在fail函数里保存第一次要保存的数据，
						key: 'title',
						success: function(res1) { //回调成功则说明不是第一次保存数据，
							for (var i = 0; i < res1.data.length; i++) { //过滤已收藏的数据
								if (res1.data[i] != strToObj) {
									continue;
								} else { //重复收藏则提示
									console.log("不要重复收藏" + res1.data[i]);
									uni.showToast({
										title: '请不要重复收藏',
										icon: 'none'
									})
							...
							res1.data.push(strToObj); //将新的数据添加进已保存的数据中
							uni.setStorage({ //保存到本地缓存
								key: 'title',
								data: res1.data, //将添加了新数据的数组重新保存
								success: function() {
									console.log('保存到本地成功');
									uni.showToast({
										title: '收藏成功',
